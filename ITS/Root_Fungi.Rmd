...
boxplot(coverage ~ Site, data = meta_alpha,
        ...
        ylim = c(0.7,1), col = clrs[levels(meta_alpha$Site)],
...
matched_otus <- guilds$OTU

unmatched <- read.table("data/for_funguild.guilds_unmatched.txt", header = T, sep = "\t", stringsAsFactors = F)

unmatched_otus <- unmatched$OTU

unmatched_shared <- shared[ , unmatched_otus]
matched_shared <- shared[ , matched_otus]

un_seqs <- apply(unmatched_shared, 1, sum)
matched_seqs <- apply(matched_shared, 1, sum)

matched_v_un <- data.frame("matched" = matched_seqs, "unmatched" = un_seqs)

matching_rel_abund <- matched_v_un / 200

fg_av_ra <- apply(matching_rel_abund, 2, mean)
fg_sd_ra <- apply(matching_rel_abund, 2, sd)
fg_se_ra <- fg_sd_ra/sqrt(nrow(matching_rel_abund))
