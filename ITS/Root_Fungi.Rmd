...
library(permute); library(vegan); library("dplyr"); library(knitr); library(ape); library(DAAG); library(MASS)
...
meta_alpha$X15SoilP <- as.numeric(meta_alpha$X15SoilP)
meta_alpha$X15SoilN <- as.numeric(meta_alpha$X15SoilN)
meta_alpha$X15SoilC <- as.numeric(meta_alpha$X15SoilC)
meta_alpha$X15TisN <- as.numeric(meta_alpha$X15TisN)
meta_alpha$X15TisP <- as.numeric(meta_alpha$X15TisP)
meta_alpha$X15TisC <- as.numeric(meta_alpha$X15TisC)
meta_alpha$Sat_lev <- as.factor(meta_alpha$Sat_lev)
meta_alpha$Lat <- as.factor(meta_alpha$Lat)


ef <- envfit(shared.mds, meta_alpha[ , c(6:13)], na.rm = T)
ef
...
ef <- envfit(shared.mds, meta_alpha[ , c(8:13)], na.rm = T)
ef
```
...
ordiellipse(NMDS, meta_alpha$Site == "BL", show.groups = TRUE, kind = "se", col = "blue", conf = 0.95, lty = 1, lwd = 2.0)
ordiellipse(NMDS, meta_alpha$Site == "CB", show.groups = TRUE, kind = "se", col = "red", conf = 0.95, lty = 1, lwd = 2.0)
...
mtext("Significant Differences",side = 4, line = -2, las = 1, at = 19.5, cex = 1)
mtext(c("Lineage","Site"),side = 4, las = 1, line = c(-1,3), at = 18.5, cex = 0.9)
arrows(x0 = c(0.96,1.14), y0 = 1, y1 = 17.5, angle = 0, lwd = 0.75)
mtext(c(".","ns","ns","ns","ns","ns"), las = 1, side = 4, line = 3, at = c(2.2,5,8,11,14,17), cex = c(2,1,1,1,1,1))
mtext(c("ns","ns","ns","ns","ns","ns"),side = 4, las = 1, at = c(2,5,8,11,14,17), line = 0, cex = 1)

legend(x = 0.55, y = 18, c("Non-native", "Native" ), pch = 22, col = "black", pt.bg = c("gray", "white"), bty = "n", cex = 1)
#dev.off()
...
arrows(x0 = x0_nat, x1 = x1_nat, y0 = c(1.5,4.5,7.5,10.5,13.5,16.5,19.5,22.5), angle = 90, lwd = 1.5, length = 0.03, code = 3)
arrows(x0 = x0_non, x1 = x1_non, y0 = c(2.5,5.5,8.5,11.5,14.5,17.5,20.5,23.5), angle = 90, lwd = 1.5, length = 0.03, code = 3)
axis(side = 2, las = 1, labels = colnames(top_av_genus), at = c(2,5,8,11,14,17,20,23))

mtext("Significant Differences",side = 4, line = -2, las = 1, at = 27, cex = 1)
mtext(c("Lineage","Site"),side = 4, las = 1, line = c(-1,3), at = 25, cex = 0.9)
arrows(x0 = 0.48, y0 = 1, y1 = 23, angle = 0, lwd = 0.75)

mtext("ns",side = 4, las = 1, line = 0, at =  c(2,5,8,11,14,17,20,23), cex = 1)
mtext(c("*","ns","ns","**","ns","ns","*","*"),side = 4, las = 1, line = 3, at =  c(1.25,5,8,10.25,14,17,19.25,22.25), cex = c(2,1,1,2,1,1,2,2))
...
reg_n <- lm(ma_finite$X15SoilN ~ NMDS1 * NMDS2)
reg_n
summary(reg_n)
